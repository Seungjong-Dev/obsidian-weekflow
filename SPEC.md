...
- 블록 우클릭 시 컨텍스트 메뉴 표시 (Obsidian 네이티브 `Menu` 사용):
  ...
  - **Return to inbox** — 블록을 인박스로 되돌리기 (Undo 가능). 모바일 터치 환경에서는 액션 바 "더보기(⋯)" 메뉴를 통해 접근
...
- 블록 탭 → 선택 모드 진입
- Apple Pencil 호버 → 선택 모드 진입 (`pointerenter` + `pointerType === "pen"`). 펜슬을 떼면(`pointerleave`) 선택 해제
...
- 다른 블록 탭 → 선택 전환
- Apple Pencil 호버로 진입한 경우: 펜슬을 떼면 선택 해제, 호버 중 탭하면 터치 선택으로 전환 (펜슬을 떼도 유지)
...
- Apple Pencil도 이동 모드에서 드래그/리사이즈 가능 (이동 모드 진입은 액션 바 탭 필요). 이동 모드 중 펜 셀 선택은 차단됨
...
- 탭 → 선택 (nav 아이콘 등 액션 표시)
- 선택된 아이템에서 액션 수행 (이동 → 그리드로 드래그, 노트 이동 등)
- 그리드 영역 또는 다른 곳 탭 → 선택 해제 (`deselectAll()`)
- 터치 환경에서 `contextmenu` 이벤트 무시 (`lastPointerType === "touch"` 가드) — 롱프레스 시 불필요한 컨텍스트 메뉴 방지
...
- 터치 블록 인터랙션: 선택 모드(액션 바) → 이동 모드(드래그/리사이즈) → 확인/취소. 롱프레스 제거, 명시적 버튼 기반. 이동 모드에서 고스트 블록에 리사이즈 핸들 자동 추가, 여러 번 드래그/리사이즈 반복 가능. 취소 시 누적 위치 원래 값으로 리셋. Apple Pencil도 이동 모드에서 드래그/리사이즈 지원 (이동 모드 중 셀 선택 차단)
...
- **초기 상태:** 양쪽 모두 접힌 상태(`earlyFolded = true`, `lateFolded = true`)
- **fold bar 구조:** 라벨 셀(1열) + 날짜별 셀(각 요일 컬럼)로 분리
  - **라벨 셀:** `▴/▾ HH:MM–HH:MM` 형태로 접힌 시간 범위와 화살표 방향 표시
  - **날짜 셀:** 해당 시간대에 숨겨진 블록이 있으면 `▴ [N]` 형태로 화살표 + 카운트 배지 표시. 숨겨진 블록이 없으면 빈 상태
- **화살표 방향:** 접힌 상태에서는 바깥쪽(상단=▴, 하단=▾, "펼치기" 의미), 펼친 상태에서는 안쪽(상단=▾, 하단=▴, "접기" 의미)
- **클릭 토글:** 라벨 셀, 날짜 셀 모두 클릭 시 해당 영역의 fold 상태 토글
- **자정 넘김 블록:** 자정을 기준으로 분할되므로, 접기 영역에 걸치는 블록은 분할된 부분만 숨겨진 블록 카운트에 반영
- **행 높이:** fold bar는 고정 높이(28px). 접힌 시간대의 행들은 렌더링에서 제외되어 그리드 공간을 차지하지 않음
...
- Pointer Events API: 모든 인터랙션을 `pointerdown`/`pointermove`/`pointerup`으로 통합 (마우스+터치+펜). `setPointerCapture()`로 리사이즈 드래그 안정화. 터치에서는 선택 모드 → 이동 모드 진입 방식으로 드래그 시작 (롱프레스 미사용)
