...
2. **📋 인박스:** 설정에 등록된 인박스 소스(노트 또는 폴더)에서 미완료 체크박스(`- [ ]`)를 수집. 소스는 정적 경로이며 개수 제한 없음. 소스별 동작:
   ...
   - **폴더 소스:** 하위 `.md` 파일 전체에서 미완료 체크박스 읽기. 읽기 전용 (쓰기 불가).
   - **새 항목 추가:** 소스 목록의 순서(우선순위)에서 첫 번째 노트 소스에 기록. 노트 소스가 없으면 추가 버튼 미표시.
...
- 인박스 연동: 등록된 인박스 소스(노트/폴더)에서 미완료 체크박스 수집 (헤딩 있으면 해당 섹션만, 없으면 전체)
