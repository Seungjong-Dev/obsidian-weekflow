...
- **클릭 토글:** 라벨 셀, 날짜 셀 모두 클릭 시 해당 영역의 fold 상태 토글. 각 셀에 `aria-label`과 `title` 툴팁("Show/Hide HH:MM–HH:MM") 제공
- **자정 넘김 블록:** 자정을 기준으로 분할되므로, 접기 영역에 걸치는 블록은 분할된 부분만 숨겨진 블록 카운트에 반영
- **행 높이:**
  - **접힌 상태:** fold bar 고정 높이(28px). 접힌 시간대의 행들은 렌더링에서 제외(0px)
  - **펼친 상태:** boundary hour 행이 일반 높이(`minmax(40px, 1fr)`)로 변경. fold bar는 동일 행에 오버레이로 렌더되며, 해당 시간의 일반 셀도 함께 렌더되어 블록 표시 및 편집 가능
