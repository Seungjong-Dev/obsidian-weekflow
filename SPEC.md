...
3. **인라인 에디터:** 셀 선택 영역 위에 플로팅 입력 필드가 표시된다. 툴바에서 선택된 카테고리가 기본 적용되며, 카테고리 도트 클릭으로 드롭다운 변경 가능. `Enter`로 즉시 블록 생성, `Tab`으로 풀 모달 전환 (시간 수정 등 상세 편집), `Esc`로 취소.
...
- **표시 조건:** 오늘이 visible range에 포함되고, 현재 시간이 folded/boundary hour가 아닐 때 표시. folded/boundary hour이면 `display: none`으로 숨김
...
- **Boundary hour:** `dayStartHour - 1` (상단) 또는 `dayEndHour` (하단). fold 상태와 무관하게 항상 이 행이 fold bar/fold zone의 대표 행
- **접힌 상태 (fold bar):**
  - 라벨 셀(1열) + 날짜별 셀(각 요일 컬럼)로 분리
  - **라벨 셀:** `▴/▾ HH:MM–HH:MM` 형태로 접힌 시간 범위와 화살표 방향 표시
  - **날짜 셀:** 해당 시간대에 숨겨진 블록이 있으면 `▴ [N]` 형태로 화살표 + 카운트 배지 표시
  - 행 높이: 28px 고정. 접힌 시간대의 다른 행들은 0px
  - 클릭 시 언폴딩
- **펼친 상태 (fold zone):**
  - Boundary hour 포함 모든 fold zone 시간(0~dayStartHour-1 / dayEndHour~23)이 **일반 행과 동일하게** 렌더: 일반 높이(`minmax(40px, 1fr)`), 10분 그리드 셀, 시간 레이블
  - Fold zone 셀과 시간 레이블에 `var(--background-secondary)` 배경 적용 — 일반 시간대와 시각적으로 구분
  - Fold zone 영역 내 **아무 셀이나 시간 레이블을 클릭**하면 재접기 (별도 버튼 불필요)
  - `cursor: pointer` 적용
- **화살표 방향:** 접힌 상태에서는 바깥쪽(상단=▴, 하단=▾, "펼치기" 의미)
...
하단에 모드 인디케이터 표시: `-- NORMAL -- 14:30`, `-- VISUAL -- 14:00 → 15:30 (1.5h)`, `-- INSERT --`
...
- 커서가 접힌(folded) 시간대에 진입하면 자동 언폴딩
...
| 키 | 동작 |
|---|---|
| h / l | 10분 가로 이동 (같은 요일, 서브슬롯 단위) |
| j / k | 1시간 세로 이동 (같은 요일, 행 단위) |
| H / L | 이전/다음 요일 |
| gg | dayStartHour로 점프 |
| G | dayEndHour로 점프 |
| gt | 현재 시각+오늘로 점프 |
| i / Enter | 블록 위: 편집 모달. 빈 칸: 인라인 에디터 |
...
`gg`, `dd`, `cc`, `gt`는 pending key 버퍼(1초 타임아웃)로 처리. 첫 키 입력 후 1초 내에 두 번째 키가 오면 시퀀스 실행, 아니면 버퍼 클리어.
