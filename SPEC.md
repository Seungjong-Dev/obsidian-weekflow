...
이동 버튼 탭 시 진입. 블록을 드래그하여 시간/요일을 변경하거나, 상하 가장자리를 드래그하여 리사이즈할 수 있다.
...
- 블록 터치+이동 → 드래그 (시간/요일 변경). 이때만 `preventDefault()` + `touch-action: none`
- 상하 가장자리 터치+이동 → 리사이즈
- **확인** 탭 → 변경 적용, 선택 모드로 복귀
- **취소** 탭 → 변경 취소 (원래 위치/크기 복원), 선택 모드로 복귀
...
- 터치 블록 인터랙션: 선택 모드(액션 바) → 이동 모드(드래그/리사이즈) → 확인/취소. 롱프레스 제거, 명시적 버튼 기반
