...
| 키 | 동작 |
|---|---|
| h / l | 10분 가로 이동 — 시간 행 경계에서 인접 요일로 래핑 (XX:50 → 다음 요일 XX:00, XX:00 → 이전 요일 XX:50) |
| j / k | 1시간 세로 이동 (같은 요일, 행 단위) |
| H / L | 이전/다음 요일 (같은 시간 유지) |
| 0 | 현재 시간 행 시작 (XX:00) |
| $ | 현재 시간 행 끝 (XX:50) |
| gg | dayStartHour로 점프 |
| G | dayEndHour로 점프 |
| gt | 현재 시각+오늘로 점프 |
| i / Enter | 블록 위: 편집 모달. 빈 칸: 인라인 에디터 (Esc/Enter로 나가면 자동으로 Normal 복귀) |
