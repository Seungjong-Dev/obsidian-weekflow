...
│ G    │        │        │        │        │       │     │
└──────┴────────┴────────┴────────┴────────┴───────┴─────┘
       좌측 spacer의 세로 레이블(REVIEW / LOG) 클릭으로 모드 전환
```
...
- 좌측 spacer에 두 개의 세로 대문자 레이블(`REVIEW`, `LOG`)이 스택되어 있으며, 활성 모드는 `--text-normal`, 비활성 모드는 `--text-faint`로 표시. 클릭하면 즉시 모드가 전환되고 선택은 `Review Panel Mode` 설정에 저장된다
