...
> 작성일: 2026-02-20
> 마지막 커밋: `b4b0bd4` fix: clear tap-tap cell anchor on modal open and block selection
...
- **터치 셀 선택 (탭-탭)**: 터치 디바이스에서 드래그→스크롤/스와이프 전용으로 해방, 셀 선택은 탭-탭 방식으로 변경
  ...
  - 앵커 초기화: 모달 열기 직전에 `clearSelection()` 호출 (모달 취소 시에도 앵커 잔류 방지), 블록 탭 시에도 `clearSelection()` 호출
...
- **터치 블록 인터랙션**: 롱프레스 제거, 선택 모드(액션 바 5버튼: 편집/이동/삭제/노트/더보기) → 이동 모드(드래그/리사이즈, 확인/취소) → 삭제 확인 모드. `TouchBlockMode` 4-state 머신 (`none`/`selected`/`move`/`delete-confirm`). Apple Pencil 호버로 선택 모드 진입 지원
...
- **CSS 터치 최적화**: `@media (pointer: fine)` hover 격리, `@media (pointer: coarse)` 상시 표시 토글/핸들, `.weekflow-grid`/`.weekflow-block` `touch-action: pan-y` (블록 위에서도 스크롤 허용), `.weekflow-resize-handle`은 `touch-action: none` 유지, 터치 타겟 36px (툴바). 블록은 이동 모드 진입 시에만 동적으로 `touch-action: none` 적용, 패널 아이템은 `touch-action: pan-y`로 스크롤 허용
...
### 수동 뷰 모드 전환
- **`view.ts`**: `viewModeOverride: "auto" | 7 | 3 | 1` 상태 추가. 기본값 `"auto"`는 기존 반응형 동작 유지
  - `showViewModeMenu()`: 주차 라벨 클릭 시 Obsidian `Menu`로 7 days / 3 days / 1 day / Auto 선택. `setChecked()`로 현재 선택 표시
  - `setViewModeOverride()`: override 설정 → `currentVisibleDays` 변경 → `calculateDayOffset()` → `renderView()`
  - `onLayoutTierChanged()`: CSS 클래스는 항상 실제 너비 기반으로 갱신 (패널/바텀시트 결정), `viewModeOverride === "auto"`일 때만 `currentVisibleDays`/`currentDayOffset` 자동 변경
  - 스와이프 게이팅: `currentLayoutTier === "wide"` → `currentVisibleDays >= 7`로 변경 (override로 3일 뷰를 넓은 화면에서 사용할 때 스와이프 동작)
  - `getState()`/`setState()`: `viewModeOverride`를 Obsidian view state에 저장/복원 (세션 내 지속)
...
### 터치 블록 인터랙션 (선택 → 액션 바 → 이동/삭제)
- **`grid-renderer.ts`**: 롱프레스(300ms) 터치 드래그를 4-state 선택 모드로 교체
  - `TouchBlockMode`: `"none"` | `"selected"` | `"move"` | `"delete-confirm"`
  - `TouchBlockSelection` 인터페이스: mode, dayIndex, item, 원래 위치(복원용), Apple Pencil 호버 상태 추적
  - `dataset.itemId`를 모든 블록 세그먼트에 설정 (기존에는 오버랩 블록에만) → `findBlockElement()` 동작
  - 블록 `pointerdown`: 터치는 `preventDefault()` 미호출(스크롤 허용), 이동 모드의 선택된 블록만 즉시 드래그
  - 블록 `click`: 터치→`handleTouchBlockTap()`, 펜 호버 탭→영구 선택 전환, 마우스→기존 편집
  - `showActionBar()`: `document.body`에 `position: fixed`로 5버튼 액션 바 (편집/이동/삭제/노트/더보기), 뷰포트 경계 클램핑
  - `enterMoveMode()`: 블록에 `touch-action: none` 동적 적용, 점선 테두리, `hapticFeedback()`
  - `onBlockDragFinish()`/`onResizeDragFinish()`: 이동 모드에서 고스트 유지 (확인/취소 대기)
  - `confirmMove()`/`cancelMove()`: 이동 확정/원위치 복원
  - `enterDeleteConfirmMode()`/`confirmDelete()`/`cancelDeleteConfirm()`: 삭제 확인 흐름
  - Apple Pencil: `pointerenter`(`pointerType === "pen"`)→임시 선택, `pointerleave`→해제(탭으로 영구 전환 가능)
  - 빈 셀 탭/스크롤 시 선택 해제, `render()` 후 선택 복원, `destroy()` 시 정리
  - `GridCallbacks.onBlockDelete` 추가
- **`planning-panel.ts`**: 터치 아이템 선택 패턴
  - `pointerdown`: 터치는 `preventDefault()` 미호출 (스크롤 허용), 마우스는 즉시 드래그
  - `click`: 터치 탭→선택 토글 (`selectItem()`/`deselectItem()`)
  - 선택 시 액션 스트립 표시 (드래그 핸들 + 노트 이동 버튼)
- **`view.ts`**: `onBlockDelete` 콜백 연결, 그리드 스크롤 시 `clearTouchSelection()` 호출
- **`styles.css`**: 액션 바 (`.weekflow-action-bar`, fixed, z-index 30, 모바일 44px 터치 타겟), 블록 터치 상태 (`.weekflow-block-touch-selected`/`-move-mode`/`-delete-pending`), 패널 선택 (`.weekflow-panel-item-selected`/`-actions`), `touch-action: pan-y` (패널 아이템)
...
17. **Pointer Events 통합**: 모든 인터랙션을 `pointerdown`/`pointermove`/`pointerup`으로 구현. `setPointerCapture()`로 요소 밖 드래그 유지. 데스크톱 블록 드래그는 150ms 딜레이 분기. 터치는 선택 모드 → 이동 모드 방식으로 드래그 진입 (롱프레스 미사용). `onGlobalPointerMove`에서 `cell-select` 모드를 처리하여 터치 드래그 시 `getCellFromPoint()`로 다중 셀 선택 (`pointerenter`는 터치에서 미발생)
...
24. **수동 뷰 모드 override**: `viewModeOverride`는 view state로만 저장 (설정 아님). CSS layout tier(패널/바텀시트 결정)와 visibleDays(그리드 표시 일수)를 분리하여 override 시에도 패널 레이아웃은 실제 너비에 따라 적응. 스와이프 게이팅도 layoutTier 대신 currentVisibleDays 기반으로 판단
...
모든 설계된 기능이 구현 완료됨. 추후 사용자 피드백에 따라 개선 예정.

## 알려진 이슈 / 개선 여지

- ~~`getCellFromPoint()` 수학 계산에서 스크롤 위치 반영이 정확한지 실사용 검증 필요~~ — 해결: `getBoundingClientRect()`가 스크롤을 이미 반영하므로 `scrollLeft/Top` 이중 가산 제거
- 블록 드래그 시 150ms(데스크톱) 딜레이 + 5px 이동 임계값으로 클릭/드래그 구분 — 체감 조정 가능. 터치는 선택→이동 모드 방식으로 구현 완료
