...
> 작성일: 2026-02-18
> 마지막 커밋: `8dd3b77` fix: iPad touch drag cell selection and sidebar swipe conflict
...
- **CSS 터치 최적화**: `@media (pointer: fine)` hover 격리, `@media (pointer: coarse)` 상시 표시 토글/핸들, `touch-action: none` (개별 요소 + `.weekflow-grid` 컨테이너), 터치 타겟 44px
