...
- **터치 블록 인터랙션 (설계 완료, 구현 예정)**: 롱프레스 제거, 선택 모드(액션 바) → 이동 모드(드래그/리사이즈) → 확인/취소. SPEC.md "터치 블록 인터랙션" 섹션 참조
...
- **CSS 터치 최적화**: `@media (pointer: fine)` hover 격리, `@media (pointer: coarse)` 상시 표시 토글/핸들, `.weekflow-grid`/`.weekflow-block` `touch-action: pan-y` (블록 위에서도 스크롤 허용), `.weekflow-resize-handle`은 `touch-action: none` 유지, 터치 타겟 36px (툴바). 블록/패널 아이템은 이동 모드 진입 시에만 동적으로 `touch-action: none` 적용 (설계 완료, 구현 예정)
...
17. **Pointer Events 통합**: 모든 인터랙션을 `pointerdown`/`pointermove`/`pointerup`으로 구현. `setPointerCapture()`로 요소 밖 드래그 유지. 데스크톱 블록 드래그는 150ms 딜레이 분기. 터치는 선택 모드 → 이동 모드 방식으로 드래그 진입 (롱프레스 미사용, 설계 완료/구현 예정). `onGlobalPointerMove`에서 `cell-select` 모드를 처리하여 터치 드래그 시 `getCellFromPoint()`로 다중 셀 선택 (`pointerenter`는 터치에서 미발생)
...
### 터치 블록/패널 인터랙션 리팩토링 (설계 완료, 구현 예정)

현재 터치 블록 드래그는 롱프레스(300ms) 방식이나, 스크롤과 충돌하고 컨텍스트 메뉴를 차단하는 문제가 있다. SPEC.md에 정의된 **선택 모드 → 액션 바 → 이동/삭제 확인** 패턴으로 교체 예정:

- 블록: 탭→선택(액션 바: 편집/이동/삭제/노트/더보기) → 이동 버튼→이동 모드(드래그/리사이즈, 확인/취소) → 삭제 버튼→삭제 확인(확인/취소)
- 패널 아이템: 탭→선택(액션 표시) → 이동→그리드로 드래그
- 롱프레스 제거, 모든 동작이 명시적 버튼 탭
- 선택 전까지 `preventDefault()` 미호출로 스크롤 자유

주요 변경 파일: `grid-renderer.ts`, `planning-panel.ts`, `view.ts`, `styles.css`

## 알려진 이슈 / 개선 여지

- ~~`getCellFromPoint()` 수학 계산에서 스크롤 위치 반영이 정확한지 실사용 검증 필요~~ — 해결: `getBoundingClientRect()`가 스크롤을 이미 반영하므로 `scrollLeft/Top` 이중 가산 제거
- 블록 드래그 시 150ms(데스크톱) 딜레이 + 5px 이동 임계값으로 클릭/드래그 구분 — 체감 조정 가능. 터치는 선택→이동 모드 방식으로 교체 예정
